...
def test_run_deploy_queue_mode_command(tmp_path: Path) -> None:
    ...
    with (
        ...
        patch.object(deploy_runner, "verify_package_on_device"),
    ):
        from game_agent.services.subprocess_tree import PopenResult

        monitor = MagicMock()
        monitor.result.summary.return_value = ""
        ...
        popen_mock.return_value = PopenResult(returncode=0, stdout=b"", stderr=b"")
...
def test_finalize_merged_config_moves_from_native(tmp_path, monkeypatch) -> None:
    ...
    monkeypatch.setattr(gb, "gameturbo_merged_config_path", lambda gid: native)
    final = gb.finalize_merged_config_after_deploy("42", target)
