...
_DOWNLOAD_STRONG_CONTEXT_RE = compile_lexicon_pattern(
    Concept.DOWNLOAD_STRONG,
    Concept.RESOURCE_DOWNLOAD,
)
_DOWNLOAD_UPDATE_VERB_RE = compile_lexicon_pattern(Concept.DOWNLOAD_UPDATING)
DOWNLOAD_PROGRESS_RE = re.compile(r"\d+\s*%|%\s*\d+")
DOWNLOAD_UPDATING_RE = compile_lexicon_pattern(Concept.DOWNLOAD_UPDATING)
...
async def resolve_download_gate(
    ...
    *,
    ...
    if discard_if_stale(work_gen, where="download_gate", ctx=attempt_context):
        return apply_download_ocr_to_facts(facts, ocr_merged=gate_ocr)
