...
def fill_login_form_batch(
    ...
    *,
    ...
    """兼容别名：委托 atomic_login_fill_and_submit。"""
    ...
    return LoginBatchResult(ok=result.ok, message=result.message, targets=result.targets)


# --- 保留分步 API 供测试/recover 过渡（内部可委托原子函数） ---
...
def _load_targets(
    ...
    *,
    ...
    screen_height: int,
    tag: str,
) -> tuple[LoginFormOcrTargets, str]:
    ocr = ocr_login_field_coords(
        adb,
        artifact_root=artifact_root,
        screen_width=screen_width,
        screen_height=screen_height,
    ...
    return ocr.targets, ocr.ocr_summary


def fill_account_step(
    adb: Any,
    *,
    username: str,
    executor: ExecutorSection,
    ...
    targets, _ = _load_targets(
        ...
        screen_width=sw,
        screen_height=sh,
        tag="graph_login_account",
    )
    if targets.account_xy is None:
        return LoginStepResult(ok=False, message="no account field coords in OCR", targets=targets)
    fill_result = fill_login_at_coords(
        adb.device_serial,
        account_xy=targets.account_xy,
        password_xy=None,
    ...
    if not fill_result.ok:
        return LoginStepResult(ok=False, message=fill_result.message[:400], targets=targets)
    return LoginStepResult(ok=True, message=fill_result.message[:300], targets=targets)


def _dismiss_keyboard_if_black(
    adb: Any,
    executor: ExecutorSection,
    *,
    screen_width: int,
    ...
    sw, sh = screen_width, screen_height
    probe_path = artifact_root / f"{tag}_kb_probe.png"
    adb.screencap_png(probe_path)
    summary, bboxes = run_ocr_frame(
        probe_path, device_w=sw, device_h=sh, worker_key=adb.device_serial,
    )
    if not is_login_secure_keyboard_blackout(probe_path, bboxes, ocr_summary=summary):
        return None
    return try_dismiss_login_secure_keyboard(adb, executor)


def fill_password_step(
    adb: Any,
    *,
    password: str,
    executor: ExecutorSection,
    ...
    sw, sh = screen_width, screen_height
    kb_msg = _dismiss_keyboard_if_black(
        adb,
        executor,
        screen_width=sw,
        screen_height=sh,
        artifact_root=artifact_root,
        tag="graph_login_password",
    )
    targets, _ = _load_targets(
        adb,
        executor=executor,
    ...
    if not fill_result.ok:
        return LoginStepResult(ok=False, message=fill_result.message[:400], targets=targets)
    prefix = f"{kb_msg}; " if kb_msg else ""
    return LoginStepResult(ok=True, message=(prefix + fill_result.message)[:300], targets=targets)


def submit_login_step(
    adb: Any,
    *,
    executor: ExecutorSection,
    artifact_root: Path,
    ...
    login_xy = targets.login_button_xy or cached_login_xy
    submit_ok, msg = submit_login_after_fill(
        adb.device_serial,
        adb,
        executor=executor,
        artifact_root=artifact_root,
    ...
    if not submit_ok:
        return LoginStepResult(ok=False, message=msg[:400], targets=targets)
    prefix = f"{kb_msg}; " if kb_msg else ""
    return LoginStepResult(ok=True, message=(prefix + msg)[:300], targets=targets)
