...
def _log_tool(ctx: RunContext[KeyWizardAgentDeps], name: str, args: Any, result: str) -> None:
    if ctx.deps.audit is not None:
        ctx.deps.audit.log_tool("keywizard", ctx.deps.round_id, name, args, result)


def _prompt_path() -> Path:
    return Path(__file__).resolve().parent / "prompts" / "keywizard_system.zh.txt"


def _block_executor_if_game_running(ctx: RunContext[KeyWizardAgentDeps]) -> str | None:
    """游戏进程已启动则禁止继续点击/滑动等执行者操作。"""
...
async def _wait_for_game_process(
    ctx: RunContext[KeyWizardAgentDeps],
    *,
    summary: str,
    timeout_s: float | None,
) -> str:
    """
    「定时炸弹」：点启动后由 AI 调用；在限时内轮询 pidof，成功或超时一次定论。
    """
    cfg = ctx.deps.app_config
    game_pkg = cfg.game.package_name
    run = ctx.deps.run_state
    run.script_launch_claimed = True
    ...
    note = (summary or "已在按键精灵中触发脚本启动").strip()[:2000]
    run.note = note

    logger.info(
        "[KeyWizard] 开始等待游戏进程 %s | 超时 %.0fs | 间隔 %.1fs | %s",
        game_pkg,
        timeout,
        interval,
        note[:120],
    ...
    while time.monotonic() < deadline:
        ...
        if is_game_running(ctx.deps.adb, game_pkg):
            ...
            logger.info("[KeyWizard] %s", msg)
            if ctx.deps.audit is not None:
                ctx.deps.audit.log_phase("keywizard", "游戏进程已启动", package=game_pkg, attempts=attempt)
            return msg

        remaining = deadline - time.monotonic()
        logger.info(
            "[KeyWizard] 等待游戏进程 %s | 第 %d 次未检测到 | 剩余约 %.0fs",
            game_pkg,
            attempt,
            max(0.0, remaining),
        )
    ...
    fail_msg = (
        ...
        f"脚本触发说明：{note}。"
        "请调用 report_flow_done(success=false) 说明阻塞原因，或检查脚本/包名是否正确。"
    )
    run.note = fail_msg[:2000]
    logger.warning("[KeyWizard] %s", fail_msg)
    if ctx.deps.audit is not None:
        ctx.deps.audit.log_phase("keywizard", "等待游戏进程超时", package=game_pkg, timeout_s=timeout)
    return fail_msg


def build_keywizard_agent(app_config: AppConfig) -> Agent[KeyWizardAgentDeps, str]:
    model = build_llm_model(app_config.llm)
    system_prompt = _prompt_path().read_text(encoding="utf-8")
    default_tap_observe = app_config.agent.tap_observe_count
    agent: Agent[KeyWizardAgentDeps, str] = Agent(
        model,
        deps_type=KeyWizardAgentDeps,
        system_prompt=system_prompt,
        output_type=str,
    )

    @agent.tool
    async def list_learned_skills(ctx: RunContext[KeyWizardAgentDeps], limit: int = 15) -> str:
        """列出由历史成功 run 自动总结的技能 Markdown（文件名 + 预览）。按需调用，勿每轮盲拉。"""
    ...
    @agent.tool
    async def read_learned_skill(ctx: RunContext[KeyWizardAgentDeps], filename: str) -> str:
        """读取 experiences/agent_skills/ 下一条已学技能全文；filename 用 list 返回的 .md 名。可多次读取不同文件合并参考。"""
    ...
    @agent.tool
    async def verify_adb_connection(ctx: RunContext[KeyWizardAgentDeps]) -> str:
    ...
    @agent.tool
    async def open_keywizard_app(ctx: RunContext[KeyWizardAgentDeps]) -> str:
        """打开按键精灵 App：使用配置中的确定性 am start -n 组件，不需要模型拼 ADB 命令。"""
        blocked = _block_executor_if_game_running(ctx)
        if blocked:
            ctx.deps.view.tool("open_keywizard_app", blocked)
            _log_tool(ctx, "open_keywizard_app", {}, blocked)
            return blocked
        app = ctx.deps.app_config.keywizard
        if not app.activity.strip():
            return "配置错误：keywizard.activity 为空，按键精灵链路禁止 monkey 启动"
        msg = ctx.deps.adb.launch_game(app.package_name, app.activity)
        ctx.deps.view.tool("open_keywizard_app", msg[:800])
        _log_tool(ctx, "open_keywizard_app", {}, msg[:2000])
        return msg

    @agent.tool
    async def force_stop_app(ctx: RunContext[KeyWizardAgentDeps], package_name: str) -> str:
        """通过 adb am force-stop 结束指定包名应用（关闭游戏、按键精灵等）。"""
    ...
    async def force_stop_apps(
        ...
        """批量 force-stop 多个包名，例如同时关闭游戏与按键精灵。"""
    ...
    @agent.tool
    async def force_stop_keywizard(ctx: RunContext[KeyWizardAgentDeps]) -> str:
        """结束按键精灵进程（使用配置 keywizard.package_name）。"""
        pkg = ctx.deps.app_config.keywizard.package_name
        msg = ctx.deps.adb.force_stop_package(pkg)
        ctx.deps.view.tool("force_stop_keywizard", msg)
        _log_tool(ctx, "force_stop_keywizard", {}, msg)
        return msg

    @agent.tool
    async def capture_screenshot(ctx: RunContext[KeyWizardAgentDeps], name: str) -> str:
    ...
    async def get_ocr_summary(
        ...
        """截屏并对当前画面做 OCR，返回带中心坐标的文字列表（操作跳转后应调用本工具，勿沿用轮次开局 OCR 块）。"""
    ...
    @agent.tool
    async def tap_coordinate(ctx: RunContext[KeyWizardAgentDeps], x: int, y: int) -> str:
        """在屏幕像素坐标 (x,y) 执行点击。"""
        blocked = _block_executor_if_game_running(ctx)
        if blocked:
        ...
        out = f"{msg}\n提示：界面可能已变化，请调用 get_ocr_summary 获取最新 OCR，勿沿用本轮开局快照。"
    ...
    async def tap_and_observe(
        ...
        """
        点击后进行高频连拍观察，减少“瞬时提示”被轮次截图错过的问题。
        返回：点击结果 + 多张截图路径 + 每次 OCR 摘要（截断）。
        """
        blocked = _block_executor_if_game_running(ctx)
        if blocked:
    ...
    @agent.tool
    async def press_back(ctx: RunContext[KeyWizardAgentDeps]) -> str:
        """按系统返回键。"""
        blocked = _block_executor_if_game_running(ctx)
        if blocked:
    ...
    @agent.tool
    async def wait_seconds(ctx: RunContext[KeyWizardAgentDeps], seconds: float) -> str:
        """阻塞等待若干秒，用于下载/转圈等。"""
    ...
    @agent.tool
    async def wait_for_game_after_script_launch(
        ctx: RunContext[KeyWizardAgentDeps],
        summary: str,
        timeout_s: float | None = None,
    ) -> str:
        """
        在按键精灵里点击脚本「启动/运行」后必须调用（定时炸弹）。
        在 timeout_s 内轮询 game.package_name 进程：检测到则返回成功并结束执行者；
        超时则自动判定失败。调用前勿用 wait_seconds 代替本工具。
        """
        blocked = _block_executor_if_game_running(ctx)
        if blocked:
            ctx.deps.view.tool("wait_for_game_after_script_launch", blocked)
            _log_tool(ctx, "wait_for_game_after_script_launch", {"summary": summary[:200]}, blocked)
            return blocked
        out = await _wait_for_game_process(ctx, summary=summary, timeout_s=timeout_s)
        ctx.deps.view.tool("wait_for_game_after_script_launch", out[:1200])
        _log_tool(
            ctx,
            "wait_for_game_after_script_launch",
    ...
    @agent.tool
    async def mark_script_launched(
        ctx: RunContext[KeyWizardAgentDeps],
        summary: str,
    ) -> str:
        """已弃用：请改用 wait_for_game_after_script_launch（会自动等待并判定成败）。"""
        return await wait_for_game_after_script_launch(ctx, summary)
    ...
    async def report_flow_done(
        ...
        """遇到不可恢复阻塞时 success=false；脚本已点启动请用 wait_for_game_after_script_launch。"""
        if not success:
            ...
            out = (
                ...
                "脚本启动后请调用 wait_for_game_after_script_launch。"
            )
        ctx.deps.view.tool(
            "report_flow_done",
