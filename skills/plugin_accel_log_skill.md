---
name: plugin_accel_log
skill_id: plugin_accel_log
description: >-
  可选加速插件（GameTurbo）日志健康基线：仅 external_services.gameturbo.enabled 时用于 Modify/E2 分析。
  与核心进游戏 UI 排障无关。Index: skills/SKILL.md → read_repo_skill("plugin_accel_log").
---

# 插件加速日志健康基线（GameTurbo · AI 判断指南）

## 适用范围（必读）

| 项 | 说明 |
|----|------|
| **何时需要本 skill** | `config/settings.yaml` → `external_services.gameturbo.enabled: true`，且任务涉及 E2 网络重试、Modify、`gameturbo.log` |
| **何时不需要** | 插件关闭、纯 UI/登录/局内排障 → 读 **`game_launch_ocr`** |
| **代码位置** | `game_agent/external_services/gameturbo/`（deploy、log、retry/modify、failure_report）；调度入口 `external_services/manager.py` |
| **运行时** | Executor **不**按日志 rule 判死；日志采集 + 失败时域名提取；**Modify** 阶段 AI 读取本 skill（`retry/prompts.py`） |

`GameTurbo-Native/` 为独立仓库/目录，由插件 deploy 脚本调用，**不在**核心 `game_agent/graphs/` 内。

## When to read this file

- **Runtime:** the executor does **not** rule-judge `gameturbo.log` for fail-fast; the **plugin** collects logs. Network-like failures use OCR + multimodal during the run; **Modify** (`external_services/gameturbo/retry/`) combines archived log + `domain_region_analysis.json`.
...
Domain analysis: read artifact `domain_region_analysis.json` (plugin artifact; not shell stdout).
Generated by `game_agent.external_services.gameturbo.log.domain_extract`.
No JSON ⇒ no automated config patch in Modify (`external_services/gameturbo/retry/modify.py`).
